...
def _stitch(chunk_a, chunk_b):
    t1, v1, c1 = chunk_a
    t2, v2, c2 = chunk_b
    t_offset = t1[-1] + (t1[-1] - t1[-2]) if len(t1) > 1 else t1[-1]
    return (
        np.concatenate([t1, np.asarray(t2) + t_offset]),
...
class WgfmuBaseProcedure(BaseProcedure):
    ...
    @property
    def top_channel(self) -> WGFMUChannel:
        return WGFMUChannel(self.top + 200)

    @property
    def bottom_channel(self) -> WGFMUChannel:
        return WGFMUChannel(self.bottom + 200)
